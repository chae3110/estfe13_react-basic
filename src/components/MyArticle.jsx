function MyArticle({ title, desc, difficulty, onChangeMode, onDelete }) {
  ...
  return (
    <section className="card mt-3 mb-3">
      ...
      {onChangeMode && <button className="btn btn-secondary btn-sm" onClick={onChangeMode}>수정</button>}
      {onDelete && <button className="btn btn-danger btn-sm" onClick={onDelete}>삭제</button>}
      </div>
    </section>
  );
}
